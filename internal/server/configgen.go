...
func (c *SetupConfig) defaults() {
	...
	// Preserve a validated Smart selection; invalid/empty/legacy values
	// normalize to stable Meta without clobbering a valid core type.
	c.MihomoCoreType = normalizeMihomoCoreType(c.MihomoCoreType)
	if c.DNSOn == "" {
		c.DNSOn = "127.0.0.1"
	}
	if c.DNSOff == "" {
...
func (a *App) ensureDefaultConfigs() error {
	...
	cfg.defaults()
	if err := a.ensureRuntimeTemplateDefaults(false); err != nil {
		return err
	}
...
%s
proxy-groups:
  - {name: Proxies, type: select, proxies: [Manual, Direct]}
  - {name: Manual, type: select, proxies: [DIRECT], include-all: true, include-all-proxies: true, include-all-providers: true}
...
rules:
  ...
  - IP-CIDR,8.8.8.8/32,Proxies
  - IP-CIDR,1.1.1.1/32,Proxies
  - MATCH,Final
