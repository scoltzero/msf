  ...
  return (
    <ModalViewport onClose={requestClose}>
      <GlassDialog className="flex max-h-[calc(100dvh-2rem)] w-full max-w-3xl flex-col p-0" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 border-t border-border/45 px-5 py-3"><p className="text-[11px] text-muted-foreground">保存会自动复验候选配置；校验不会写入或重启。</p><div className="flex items-center gap-2"><button type="button" onClick={onValidate} disabled={disabled || actionDisabled || validating || saving} className="inline-flex h-9 items-center gap-1.5 rounded-xl bg-background/50 px-3 text-xs font-medium text-foreground hover:bg-background/75 disabled:opacity-50">{validating ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Check className="h-3.5 w-3.5" />}校验配置</button><button type="button" onClick={onSave} disabled={disabled || actionDisabled || saving || validating} className="inline-flex h-9 items-center gap-1.5 rounded-xl bg-primary px-3 text-xs font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50">{saving ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : null}保存并重启</button></div></div>
      </GlassDialog>
    </ModalViewport>
  );
}
