...
export function formatMihomoMetaVersion(value: unknown) {
  const match = String(value || "").match(/v?\d+\.\d+\.\d+/i)?.[0];
  if (!match) return "-";
  return match.toLowerCase().startsWith("v") ? match : `v${match}`;
}
...
export default function MihomoConfigPage() {
  ...
  const metaVersion = useMemo(() => formatMihomoMetaVersion(version), [version]);
    ...
    <AppShell>
      <div className="space-y-4 animate-fade-in">
        ...
        <div className="overflow-hidden rounded-xl border border-border/60 bg-card shadow-sm">
          ...
          {loading ? (
            <div className="flex h-[560px] items-center justify-center bg-[#1e1e1e] text-sm text-[#d4d4d4]">
              正在加载配置...
            </div>
          ) : (
            <YamlEditor
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div className="rounded-xl border border-border/60 bg-card p-4 shadow-sm">
            ...
            <div className="font-mono text-sm text-foreground">{metaVersion}</div>
