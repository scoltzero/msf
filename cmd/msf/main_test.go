...
func TestDefaultDataDirUsesMSFEnv(t *testing.T) {
...
func TestUninstallRuntimeKeepDataAndPurge(t *testing.T) {
	...
	if err := uninstallRuntime(uninstallOptions{Prefix: prefix, DataDir: dataDir, DataDirExplicit: true, KeepData: true}); err != nil {
		t.Fatal(err)
	}
	...
	if err := uninstallRuntime(uninstallOptions{Prefix: prefix, DataDir: dataDir, DataDirExplicit: true, Purge: true, Yes: true}); err != nil {
		t.Fatal(err)
	}
