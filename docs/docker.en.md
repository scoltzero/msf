...
Current stable image: `ghcr.io/scoltzero/msf:latest`
...
The release workflow publishes both the versioned tag and `latest`; the commands below use `latest` so routine upgrades do not require manual version edits.
...
Setup preflight blocks initialization when `/dev/net/tun`, `CAP_NET_ADMIN`, `CAP_NET_RAW`, or a supported Docker network mode is missing.
...
services:
  msf:
    image: ghcr.io/scoltzero/msf:latest
    container_name: msf
    network_mode: host
    cap_add:
      - NET_ADMIN
...
```text
MSF_IMAGE=ghcr.io/scoltzero/msf:latest
...
The repository provides the Community Applications Docker template `packaging/unraid/ca/msf-docker.xml`. Installing **MSF Docker** from CA pre-fills the following settings; you can still create the container manually in Unraid Dockerman.
...
2. Install **MSF Docker** from CA, or create a container and set the image to `ghcr.io/scoltzero/msf:latest`.
...
|---|---|---|
| `MSF_IMAGE` | `ghcr.io/scoltzero/msf:latest` | Container image |
