...
这个实验镜像不会推送到 `latest`。拉取或部署 Docker 实验版时必须显式写出 `v0.3.7` tag。
...
Docker TUN 模式下，Mihomo 配置会启用 `tun.auto-route`、`tun.auto-detect-interface` 和 `tun.dns-hijack`，并显式禁用 `tun.auto-redirect`。这意味着 MSF 不会写宿主机 `table inet msf`、`fwmark 1 table 100`、`ip rule` 或 `ip route`。
...
- 镜像：`ghcr.io/scoltzero/msf:v0.3.7`
- 网络：`host`
...
2. 新建容器，镜像填写 `ghcr.io/scoltzero/msf:v0.3.7`。
...
| 变量 | 默认值 | 用途 |
|---|---|---|
| `MSF_IMAGE` | `ghcr.io/scoltzero/msf:v0.3.7` | 容器镜像 |
