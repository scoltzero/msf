...
| `launch` | UI 入口 | `Settings/msf` | ✅ 与 `.page` 的 `Menu="Settings"` 对应 |
| `icon` | 图标 | `network-wired`（Font Awesome） | ✅ |
...
- `msf.page`：`Menu="Settings:45"` + `Title="MSF Free"` + `Icon="network-wired"`，PHP 读写 `msf.cfg`、调用 rc 脚本、展示状态、给「Open WebUI」直链。✅ 标准写法。
...
6. **图标**：CA 列表用图（建议提供一个可公网访问的 PNG/SVG，区别于 `.page` 里用的 Font Awesome `network-wired`）。
