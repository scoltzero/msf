...
Publish the generated `.txz` package and `.plg` file to the GitHub release named by `RELEASE_TAG`. The immutable Release asset is the only supported plugin manifest source.

Example:

```bash
gh release create v0.3.7 \
  dist/unraid/msf-0.3.7-x86_64-1.txz \
  dist/unraid/msf.plg \
