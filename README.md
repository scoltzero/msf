...
<p align="center">
  <a href="https://github.com/zAhYAng/msf/releases/latest"><img src="https://img.shields.io/badge/Download-Latest%20Release-0969DA?style=for-the-badge&logo=github&logoColor=white" alt="下载最新版本"></a>
...
| 能力 | 说明 |
|---|---|
| 一体化控制 | 在一个 WebUI 中管理 MosDNS、Mihomo、规则、连接、日志、设备流量和系统状态。 |
| 可审计 | 配置、来源、运行状态和更新过程保持可见，不隐藏底层服务行为。 |
| 独立实现 | Go 后端、React WebUI、Mizar 品牌和 Linux 控制逻辑均在本仓库持续维护。 |
| Linux 专用 | 提供 Linux amd64 tarball 与 systemd 安装，支持 nftables 或 TUN。 |
...
| 平台 | 状态 | 安装文档 | 更新与卸载 |
|---|---|---|---|
| Linux amd64 tarball / systemd | 稳定 | [Linux 安装](docs/install/linux.md) | `msf update` / `msf uninstall` |

## 快速开始

1. 从 [Latest Release](https://github.com/zAhYAng/msf/releases/latest) 下载 Linux amd64 安装包。
2. 按 [Linux 安装文档](docs/install/linux.md) 完成安装。
3. 打开 `http://<服务器IP>:7788`，完成四步初始化向导。
4. 在你拥有或已获授权的路由器上配置 DHCP DNS 与 Fake-IP 静态路由。

### 初始化组件安装

- MosDNS 通过用户提供的发布 ZIP 安装，可选择本地上传或填写 ZIP 下载链接。ZIP 必须同时包含 MosDNS 与 `mosdns-traffic-agent`。
- MosDNS 与 `mosdns-traffic-agent` 作为一个组件组安装、启动、停止和重启；WebUI 通过本地服务代理展示设备流量数据。
- 代理核心仅使用 Mihomo，支持订阅、手动节点、nftables 转发或 TUN 模式。
- 初始化完成后，可在 MosDNS 菜单打开“流量监控”，查看局域网设备的实时上传、下载和连接状态。
...
默认链路组合 MosDNS `:53`、Mihomo DNS `:6666`、Fake-IP，以及 TProxy / Redirect 或 TUN。具体模式由 Linux 初始化设置决定。
...
- Linux tarball / systemd 可使用 `msf update` 与 `msf uninstall`。
...
MSF 在早期开发过程中参考了 [`baozaodetudou/mssb`](https://github.com/baozaodetudou/mssb) 公开代码所呈现的 MosDNS + Mihomo 功能流程，并使用 Go 对管理后端和控制平面进行了跨语言重构、重新设计、优化和扩展。mssb 主要采用 Shell/Python 脚本和配置文件组织安装及运行流程；MSF 当前实现采用独立的 Go 服务架构、数据库模型、HTTP API、配置事务、状态恢复和组件管理逻辑。
...
<p align="center">
  <a href="https://github.com/zAhYAng/msf/stargazers"><img src="https://img.shields.io/badge/Support-Give%20a%20Star-181717?style=for-the-badge&logo=github&logoColor=white" alt="为 MSF 点 Star"></a>
